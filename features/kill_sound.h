...
namespace KillSound
{
    ...
    inline void LogSoundName(const char* name)
    {
        ...
        // strncpy is fine here — buffer is fixed-size, we want truncation.
        size_t i = 0;
    ...
    //
    // The two gates are INDEPENDENT — muteValve no longer requires
    // enabled=true. This lets users pick:
    //   muteValve=1 enabled=1  → our ding only (default “custom” feel)
    //   muteValve=1 enabled=0  → silent kills (no ding at all)
    //   muteValve=0 enabled=1  → our ding stacked on top of Valve's
    //   muteValve=0 enabled=0  → stock Valve behavior (hook is no-op)
    //
    ...
    inline __int64 __fastcall hkKillFeedback(__int64 a1, __int64 a2)
    {
        ...
        // Pass through — Valve's ding still plays
        if (oKillFeedback)
    ...
    // -----------------------------------------------------------
    // Per-HIT damage feedback detour — sub_18081ED00 dispatches:
    ...
    // HS branch, we return immediately — NOTHING else in this function
    ...
    // emit through completely different code paths — nuking this
    ...
    // engine state) live in here — it is purely the sound dispatcher.
    // -----------------------------------------------------------
    ...
    // -----------------------------------------------------------
    // Universal sound-emit detour — sub_180B62270 (EmitSoundByHandle)
    ...
    //
    // Name list is intentionally narrow — we ONLY drop the headshot
    ...
    // (sub_1801B7AD0 — the unified queued entrypoint).
    //
    ...
    inline bool Setup()
    {
        ...
        __try
        {
            ...
            // — if FindPattern returns 0 we silently skip; the kill
