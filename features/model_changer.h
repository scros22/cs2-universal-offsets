...
// ---------------------------------------------------------------
// Model Changer — client-side player model swap.
//
...
// an 8-byte pointer to a precached CModel resource — same format
...
//   * Other players never see our swap — the model handle lives
...
// no signature for this — it's how scoreboard/agent-preview menus
...
namespace ModelChanger
{
    ...
    // CModelState::m_ModelName (CUtlSymbolLarge — points to an
    ...
    // null/garbage — caller must SEH-wrap.
    inline bool ReadModelName(uintptr_t modelState, char* out, int outCap)
    {
        ...
        __try {
            ...
            // Bounded copy — engine strings are usually < 128 bytes.
            int i = 0;
            for (; i < outCap - 1; ++i) {
    ...
    inline void Restore()
    {
        ...
        __try {
            ...
            if (!pawn || pawn != s_savedFromPawn) {
                // Pawn changed (round restart, team switch, etc.) — drop
    ...
    inline void Tick()
    {
        ...
        // Pawn pointer changed (respawn / team switch) — drop cache so
        ...
        if (!s_savedHandle) {
            ...
            if (!s_savedHandle) return;  // model not loaded yet — try next tick
        }
        ...
        if (!s_lastDonorHandle) return;  // no donor in match — leave model alone
