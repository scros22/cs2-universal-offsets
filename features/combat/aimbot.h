...
// ---------------------------------------------------------------
// Aimbot v3 Ã¢â‚¬â€ Clean humanized aim with Bezier physics.
// Smoothing 1Ã¢â‚¬â€œ100 scale. Anti-detection built into aim physics.
...
namespace Aimbot
{
    // Local logger â€” debug builds only. Release: no disk I/O.
#ifdef _DEBUG
    ...
    // ---------------------------------------------------------------
    // Config Ã¢â‚¬â€ clean settings. No miss chance, no HS% manager, no
    ...
    struct Config
    {
        ...
        float fov             = 3.5f;        // degrees â€” tighter cone, less obvious flicks across screen
        float smoothing       = 28.0f;       // 1=instant, 100=max drag â€” humanlike drag (was 12)
        ...
        bool  silentAim       = false;       // DISABLED â€” WriteSubtick path is unreliable for users; menu toggle removed.
        ...
        // logically dead â€” but SavedConfig uses raw fwrite/fread of the
    ...
    // Forward decl â€” Rage::EstimateDamage (defined just below) needs to
    ...
    // ---------------------------------------------------------------
    // Rage Mode â€” Neverlose / Memesense-style danger toggles.
    //
    // Kept as a SEPARATE namespace (NOT inside Aimbot::Config) so it
    // doesn't bleed into the SavedConfig fwrite blob â€” every byte after
    ...
    // something obvious" intent â€” not a config you carry around.
    //
    // What each toggle does at runtime:
    //   enabled        â€“ master switch. Gates every other rage knob.
    //   forceWallbang  â€“ ignore the visibility check (m_bSpottedByMask
    //                    + dormancy still gate the target â€” we never
    //                    aim at server-occluded entities). Pairs well
    //                    with high-pen weapons (AK/AWP/SCAR).
    //   forceBaim      â€“ aim chest (bone 23) instead of head; survives
    //                    HS-priority. Useful when target HP > 100 (HE
    //                    + helmet) and head wouldn't one-shot.
    //   lowestHpFirst  â€“ target picker uses HP as primary key instead
    //                    of FOV. Cleans up the round when multiple
    //                    enemies are visible at once.
    //   alwaysOn       â€“ clobbers cfg.aimKey to "always" on enable so
    //                    the cheat fires without a held mouse button.
    //   silentForce    â€“ clobbers cfg.silentAim=true on enable so we
    //                    never fall back to the visible-mouse path.
    //   instant        â€“ clobbers cfg.smoothing to 1.0 (no drag) on
    //                    enable. Disable for a "smooth rage" feel.
    //   minDamage      â€“ heuristic damage gate (0 = off). Targets we
    //                    estimate would deal less than this are
    //                    skipped. We don't sim FireBullets â€” the gate
    //                    is a cheap distance/HP/weapon-class check.
    //   prioritizeArmored â€“ pick helmet+armor targets last so we don't
    ...
    // change to the aim physics â€” only target-selection + safety
    ...
    namespace Rage
    {
        ...
        // Rage is "danger" tier â€” when the user flips it on we apply
        // the matching aim-physics toggles immediately (silent + always
        // on + instant smoothing). When they flip it back off, we only
        // restore the aim-physics knobs we forced â€” the rest of their
        ...
        // Damage estimator â€” reads the local pawn's active-weapon VData
        // and runs the same formula the server uses to compute final HP
        // damage:
        //
        //   visible:    dmg = baseDmg * pow(rangeModifier, dist/500)
        //   headshot:   dmg *= headshotMultiplier      (â‰ˆ 4.0 rifles)
        //   armored:    dmg *= armorRatio              (â‰ˆ 1.0..2.0)
        //   wallbang:   dmg *= 0.55                    (heuristic â€” we
        ...
        // means we have no data â€” caller should treat that as "pass".
                                  ...
                                  bool isHead, bool throughWall)
        {
            ...
            // back as nonsense floats â€” skip rather than estimate huge.
            ...
            // chest/stomach/arms). Use a coarse heuristic â€” if target
        ...
        // the shot would meet the configured min-damage threshold â€”
                                     ...
                                     bool isHead = false, bool throughWall = false)
        {
            ...
            if (est <= 0) return true;               // no data â€” don't filter
    ...
    // ---------------------------------------------------------------
    // Stat Governor Ã¢â‚¬â€ behavioral anti-detection layer.
    ...
    namespace Governor
    {
        ...
        {
            // Enabled by default 2026-04-28 â€” a 20h MM cooldown landed
    ...
    // ---------------------------------------------------------------
    // Human Neuromotor Model Ã¢â‚¬â€ mimics real biomechanical mouse control
    //
    ...
    //
    //   1. Reaction delay before aim starts (150Ã¢â‚¬â€œ300ms)
    //   2. Bell-curve velocity (accel Ã¢â€ â€™ peak Ã¢â€ â€™ decel), not stepÃ¢â€ â€™decay
    //   3. Low-freq Perlin noise tremor (2Ã¢â‚¬â€œ8 Hz), not white noise
    ...
    // Natural hand tremor: 2Ã¢â‚¬â€œ8 Hz Perlin noise, scaled by humanization
    ...
    // ---------------------------------------------------------------
    // Aim state machine Ã¢â‚¬â€ models full human aim lifecycle
    //
    //   IDLE Ã¢â€ â€™ REACTING Ã¢â€ â€™ ATTACKING Ã¢â€ â€™ CORRECTING Ã¢â€ â€™ LOCKED
    //          Ã¢â€ â€˜ 150-300ms  Ã¢â€ â€˜ bell-curve  Ã¢â€ â€˜ damped    Ã¢â€ â€˜ tracking
    ...
    struct AimState
    {
        ...
        float     accelShape     = 0.f;    // bias for acceleration phase [0.3Ã¢â‚¬â€œ0.6]
        float     peakVelMul     = 0.f;    // peak velocity multiplier [0.8Ã¢â‚¬â€œ1.2]
        float     overshootPct   = 0.f;    // overshoot as fraction of distance [0.04Ã¢â‚¬â€œ0.18]
        float     overshootDirP  = 0.f;    // overshoot direction pitch
        float     overshootDirY  = 0.f;    // overshoot direction yaw
        float     dampingRate    = 0.f;    // how fast oscillation decays [0.4Ã¢â‚¬â€œ0.7]
    ...
    // SilentAim runtime diagnostics â€” incremented inside hkCreateMove
    ...
    inline DWORD diag_silentArmed   = 0;   // hasTarget && safeCount>0 â†’ about to write
    ...
    inline float sessionSpeedBias  = 1.0f;  // [0.85Ã¢â‚¬â€œ1.15] varies over engagements
    ...
    // Maps smoothing 1Ã¢â‚¬â€œ100 to per-tick movement fraction
    //   s=1  Ã¢â€ â€™ 0.77  (nearly instant)
    //   s=30 Ã¢â€ â€™ 0.10  (medium follow)
    //   s=100Ã¢â€ â€™ 0.032 (slow drag)
    ...
    //   s=1  Ã¢â€ â€™ ~32   (instant flick)
    //   s=30 Ã¢â€ â€™ ~7.5  (natural arm speed)
    //   s=100Ã¢â€ â€™ ~3.9  (slow drag)
    ...
    //   0%Ã¢â€ â€™30%: acceleration (muscles engaging)
    //   30%Ã¢â€ â€™60%: peak velocity (full speed)
    //   60%Ã¢â€ â€™100%: deceleration (braking toward target)
    //
    // We model this as a modified smoothstep with adjustable peak
    // position. The integral gives position along the flick path.
    // ---------------------------------------------------------------

    // Velocity at time t (0Ã¢â€ â€™1), peakAt controls shape [0.3Ã¢â‚¬â€œ0.6]
    // Returns 0Ã¢â€ â€™1 velocity magnitude (not position)
    ...
    // Integrated position along flick path (0Ã¢â€ â€™1)
    ...
    inline void BeginEngagement(uintptr_t target, float fovDist)
    {
        ...
        // Per-engagement aim bias: Â±0.18Â° pitch, Â±0.22Â° yaw. Tight enough
        ...
        // Attack speed: how fast the curve progresses 0Ã¢â€ â€™1
        // Distant targets take longer (realistic Ã¢â‚¬â€ bigger mouse travel)
    ...
    // ---------------------------------------------------------------
    // Visibility Ã¢â‚¬â€ layered approach for reliability AND safety
    // ---------------------------------------------------------------
    ...
    {
        // Live dormancy is the ONLY truth â€” `originalDormant` is a chams
    ...
    inline bool IsVisible(uintptr_t pawn, uintptr_t localPawn, int entityIndex = -1)
    {
        ...
        // Live dormancy only â€” see IsEntityValid for why we no longer
        ...
        // to lock onto the bot for the rest of the round).
        {
            ...
            if (Mem::Read<bool>(node + Offsets::SceneNode::kDormant))
                return false; // Dormant â€” engine isn't updating this entity
        }
        (void)entityIndex;
        ...
        // the enemy â€” way safer than no vis check (which caused a ban).
        {
    ...
    // ---------------------------------------------------------------
    // Multi-bone scanner Ã¢â‚¬â€ tries multiple bones, returns closest to
    ...
    // Weapon inaccuracy offsets â€” use canonical Offsets:: values.
    ...
    // ---------------------------------------------------------------
    // No Spread â€” zero weapon inaccuracy each tick.
    ...
    // ---------------------------------------------------------------
    // Recoil compensation Ã¢â‚¬â€ wider randomization for natural spray
    // Range 1.80Ã¢â‚¬â€œ2.20x instead of 1.94Ã¢â‚¬â€œ2.06x so spray looks imperfect
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // FindBestTarget Ã¢â‚¬â€ simplified. No HS manager, no wall penetration,
    ...
    inline Target FindBestTarget(const Math::Vec3& eye, const Math::QAngle& view, float gameTime)
    {
        ...
        for (int i = 1; i <= 64; ++i)
        {
            ...
            // Bone fallback: head/neck â†’ chest â†’ pelvis â†’ origin so we never
            ...
            {
                // Rage path â€” vis is allowed-blocked, but we still need
            ...
            // Smoke check Ã¢â‚¬â€ VACnet flags smoke kills hard
            ...
            if (Rage::cfg.enabled && Rage::cfg.lowestHpFirst)
            {
                ...
                if (Rage::cfg.prioritizeArmored)
                {
                    ...
                    // on the player's item-services subobject â€” not a
                    // pawn-direct field â€” so we use armor value alone.)
    ...
    // ---------------------------------------------------------------
    // Mouse-input aimbot â€” read-only game data + SendInput mouse
    ...
    // Send a relative mouse movement â€” tries spoof first, falls back to direct
    inline void SendMouseDelta(long dx, long dy)
    {
        ...
        if (!useDirectSendInput)
        {
            __try {
                ...
                // Spoof stub corrupted â€” switch to direct permanently
                useDirectSendInput = true;
            }
        }

        // Direct fallback â€” resolve SendInput dynamically (no IAT)
    ...
    // looks hardest â€” instant detection.
    //
    ...
    namespace SilentAim
    {
        // Frame history layout within CCSGOInput object â€” VERIFIED 14153 via IDA
        ...
        // sub-msg gated by a ConVar. The server uses SHOOT angles for hit verification â€”
        ...
        // sub_180C53DB0 â€” CCSGOInput::WriteSubtickFromEntry (build 14154,
        ...
        // consecutive subticks. A jump of "any angle â†’ headbox in one
        ...
        inline float maxFlickPerSubtickDeg = 28.0f; // hard cap (impossible above ~35Â°)
        ...
        //                 already considers us hovering them â€” a flick
                                                 ...
                                                 double a4, int a5, __int64 a6)
        {
            ...
            // correct shoot direction â€” rewriting it here would create the
            ...
            __try {
                ...
                // (e.g. behind player), DO NOT rewrite â€” a 180Â° snap is the
                ...
                // silent â€” server sees at most one humanly-plausible flick
                ...
                // Per-shot wobble: Â±0.10Â° independent jitter so consecutive
        ...
        inline double __fastcall hkCreateMove(__int64 a1, unsigned int a2, __int64 a3)
        {
            ...
            // m_angEyeAngles inside CreateMove â€” this is the only point in
            ...
            // hook on sub_180C54450 â€” the per-subtick cmd writer).
            ...
            // containing the live view angles â€” anything we wrote
        ...
        inline bool Install()
        {
            // Try multiple patterns in order â€” CCSGOInput::CreateMove changes
            ...
            // on the very first line of hkWriteSubtick â€” but the inline
        ...
        {
            // -----------------------------------------------------------
            // sub_180C54450 â€” per-subtick cmd writer. Hooking this is
            ...
            static const char* writeSubtickSigs[] = {
                // Build 14152 â€” uniquely matches sub_180C53DB0
    ...
    inline void Tick()
    {
        ...
        // edge-detected via internal static â€” no-op unless the user just
        ...
        if (cfg.silentAim != prevSilentAim)
        {
            ...
            // Keep SilentAim::hasTarget at whatever it currently is â€”
        ...
        // anyone is watching us â€” without paying the entity-list scan
        // cost on every subtick. (Reverse-engineered from client.dll â€”
        ...
        // booleans â€” flipping our behavior on them is exactly what the
        // server expects of a real player.
        {
            ...
            // tell â€” every legit player has both hands on the kit.
            if (!suppress && lpHard) {
                __try {
            ...
            // ---- MoveType gate -------------------------------------
            // Only WALK (2) and FLYGRAVITY (4 â€” airborne) are normal
            ...
            if (!suppress && lpHard) {
                ...
                if (wep) {
                    ...
                    // == 0 cannot produce a bullet â€” the server-side fire
                    ...
                    // produce a bullet from any attack input â€” but our
        ...
        // hkWriteSubtick throttle â€” both reads cost a single deref each
            ...
            if (lp) {
                __try {
                    ...
                    if (id > 0 && id < 65536) {
                        ...
                        // entity is under the crosshair â€” for a player target
                    ...
                    // are server-replicated â€” reading them is identical to
                    // any other ESP read.
...

        {
            ...
            // Jump-shot policy:
            //   * jumpShot OFF              â†’ never aim while airborne (default safe).
            //   * jumpShot ON               â†’ only aim at the JUMP APEX (vzâ‰ˆ0)
            ...
            //   * jumpApexOnly checkbox     â†’ kept as a redundant gate for
            ...
            // Rare micro-pauses (~1%) â€” keeps pattern non-deterministic
            ...
            __try {
                ...
                t = Target{}; // Corrupt entity data â€” treat as no target
            }
            ...
            if (!t.pawn)
            {
                ...
                // flickers (smoke, lag, tight angles) â€” only lose target after
                ...
                SilentAim::targetPawn = 0;
                SilentAim::hasTarget = false;
                haveLastValidAim = false;  // full reset â†’ drop stale angle too
                goto tickDone;
            }

            // Target found â€” reset no-target counter
            noTargetTicks = 0;
            ...
            else if (t.pawn != state.lockedTarget)
            {
                ...
                if (state.lockedTarget != 0 && state.phase != PHASE_IDLE)
                {
                    __try {
                        ...
                        // Stale pointer â€” accept new target, don't crash-loop
                    }
                }
            ...
            {
                // Set target angle directly â€” CreateMove hook will apply it.
            ...
            // (CS2 raw-input ignores SendInput â†’ camera may not move, but bullets
            ...
            // linearly attenuates down to 55% speed.
            {
                ...
                if (t.fov > innerZone && fovEdge > 0.01f)
                {
                    ...
                    float atten = 1.0f - edgeFrac * 0.45f; // 1.0 â†’ 0.55
                    moveDp *= atten;
                    moveDy *= atten;
                }
            }
            ...
            // Real arms have mass â€” you can't instantly change direction.
            ...
            // entry instead via the CreateMove hook â€” they are MUTUALLY
            ...
            if (!cfg.silentAim)
            {
                ...
                // INTENTIONALLY NO goto â€” let SendInput humanization run too.
            }
    ...
    tickDone:;
        ...
        consecutiveCrashes = 0; // Reached end of tick without crashing
            ...
            // Backoff intentionally NOT armed â€” see top of Tick(). The
    ...
    inline bool Setup()
    {
        ...
        if (!SilentAim::Install())
            AimLog("[Aimbot] Silent aim hook failed â€” mouse-only mode available");
