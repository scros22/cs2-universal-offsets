#pragma once
// ---------------------------------------------------------------
// InventoryChanger — main-menu locker injection
...
//   2) GetLocalInventory() vfunc → CCSPlayerInventory*.
...
//        - inventory->AddEconItem(p)  (uses GetSOCache → CreateBaseTypeCache → AddObject + SOCreated)
...
{
    // ---------------------------------------------------------------
    // Tiny logger — appends to %TEMP%\inventory_changer.log
    // ---------------------------------------------------------------
...
#pragma pack(pop)
    ...
    // ---------------------------------------------------------------
    // Tracking — items we injected so we can remove + so the GetItemInLoadout
    ...
    // ---------------------------------------------------------------
    // GetItemInLoadout hook — when the locker UI asks the inventory
    ...
    // ---------------------------------------------------------------
    // ID generation — we write synthetic high-bit IDs so they never
    ...
    // ---------------------------------------------------------------
    // Init — resolve sigs + install MinHook on GetItemInLoadout
    // ---------------------------------------------------------------
    ...
    inline bool Init()
    {
        ...
        // 1) CCSInventoryManager::GetInstance — it's a `call rel32` site
        ...
        if (pCall)
        {
            ...
            IcLog("[InvCh]   resolved → 0x%llX", (unsigned long long)target);
        }

        // 2) CGCClientSharedObjectCache::CreateBaseTypeCache — direct function
        ...
        // 3) CEconItem::CreateInstance — function preamble pattern
        ...
        // 4) CCSPlayerInventory::GetItemInLoadout — direct function, hook target
        ...
        if (pGIL)
        {
            ...
            IcLog("[InvCh] MH_CreateHook(GetItemInLoadout) → %d", (int)st);
            if (st == MH_OK || st == MH_ERROR_ALREADY_CREATED)
            {
                MH_STATUS st2 = MH_EnableHook(g_pGetItemInLoadoutTarget);
                IcLog("[InvCh] MH_EnableHook → %d", (int)st2);
            }
        }

        // 5) SetDynamicAttributeValue — long sig from ARCHILIX
    ...
    // ---------------------------------------------------------------
    // Auto-inject support — called every frame from SkinChanger::Tick
