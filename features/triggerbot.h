...
// =====================================================================
//  Seeded Triggerbot — calls Valve's spread RNG functions directly to
...
//    5. ray-test dir vs (bone_pos - eye, hitbox_radius). hit ⇒ click.
//
...
//   * Tier0's RandomSeed is called by the engine spread fn itself —
...
{
    // -------------------------------------------------------------
    // Public configuration (menu-bound — names preserved for compat)
    // -------------------------------------------------------------
    struct Config
    {
        bool  enabled       = false;
        ...
        // that's 12.5 units — enough to miss a head sphere. Lead the
        ...
        // {tick-1, tick, tick+1, tick+2} — each candidate gets its own
        ...
        //      can't run — sigs not resolved or args cache empty) ----
        bool  accuracyGate    = true;
        float maxPenalty      = 0.45f;
        bool  speedGate       = false;      // off by default — predictor handles it
    ...
    // Sigs (shared with seed_probe.h — duplicated here so the probe
    ...
    // -------------------------------------------------------------
    // Per-itemDef args cache — populated by passive hook on spread fn.
    ...
    // we override (always 1 for prediction — we only test where the
    ...
    // -------------------------------------------------------------
    // Passive spread hook — captures last-known args per weapon.
    ...
    // -------------------------------------------------------------
    // Setup — sig-scan client.dll for both engine functions and
    ...
    //   up      = forward × right  (so dir = fwd + right*X + up*Y)
    //
    ...
    // right = (-sy, cy, 0) — verified algebraically against the IDA
    ...
    // -------------------------------------------------------------
    // PredictHit — the heart of the seeded trigger. Runs the engine's
                           ...
                           float radius)
    {
        ...
        // we just hand it the raw values — wrapper sub_180C75F00 is
        ...
        // come out of the barrel — fixes jumpshot / move-shot accuracy.
        ...
        // Distance from infinite ray (origin=eye, dir already ≈ unit
    ...
    inline void Tick()
    {
        ...
        // Crosshair target — m_iIDEntIndex is server-authoritative
        ...
        // delta inside a tight cone instead — same approach the
        ...
        if (predictionWanted)
        {
            ...
            // out forever. Cold cache → fall through to legacy gates
            ...
            if (cacheHot)
            {
                ...
                // m_vecVelocity is the post-friction tick velocity —
                ...
                if (!hit) return;     // not this tick — keep waiting
            }
            else
            {
                static std::atomic<std::uint16_t> s_lastLogged{0xFFFF};
                if (s_lastLogged.exchange(itemDef) != itemDef)
                    DLog("[predict] cache COLD itemDef=%u — falling through to fire so cache seeds",
                         (unsigned)itemDef);
            }
        }

        // Legacy accuracy/speed gates — only run when prediction
        ...
        if (burstLeft > 0)
        {
            ...
            burstLeft--;
            lastShotTime = now;
            // Bates(2)-distributed cooldown — looks human, not uniform
