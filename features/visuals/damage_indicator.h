...
// ---------------------------------------------------------------
// Damage Indicator â€” on-screen feed showing damage dealt/received.
...
namespace DamageIndicator
{
    ...
    // ---------------------------------------------------------------
    // Floating damage numbers â€” appear on-screen at the player hit
    // ---------------------------------------------------------------
    struct FloatingDmg
    {
        uintptr_t pawn;
    ...
    // ---------------------------------------------------------------
    // Tick â€” run every frame, detect health deltas
    // ---------------------------------------------------------------
    inline void Tick()
    {
        ...
        __try {
            ...
            // Shot correlation â€” track when we fire
            ...
            // First tick after spawn/respawn â€” snapshot all health, skip detection
            if (!trackInit)
            {
                trackInit = true;
                localPrevHp = localHp;
            ...
            for (int i = 1; i <= 64; ++i)
            {
                ...
                // Player changed (disconnect/reconnect) â€” reset tracking
    ...
    // ---------------------------------------------------------------
    // Render â€” draw the damage feed on screen
    // ---------------------------------------------------------------
    inline void Render()
    {
        ...
        switch (cfg.style)
        {
        case 1:  // Minimal â€” smaller, no prefix
            fs = ImGui::GetFontSize();
            lineH = fs + 4.f;
            break;
        case 2:  // Bold â€” larger
        ...
        if (!entries.empty())
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                if (cfg.style == 1)
                {
                    // Minimal style â€” no prefix, just the message
                    OutlinedText(dl, font, fs, ImVec2(x, y), textCol, e.text);
                }
                else
                {
                    // Classic / Bold â€” "[License] " in orange, then message
