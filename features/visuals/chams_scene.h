...
// ---------------------------------------------------------------------------
// chams_scene.h â€” Phase 1 of GeneratePrimitives chams migration.
//
// GOAL (multi-phase):
...
//       and pick PER-ENTITY (friend/enemy/local) material â€” the D3D11
...
//       a real model â€” no ad-hoc pixel shader.
//
...
//   - Detour body is a PURE PASSTHROUGH â€” calls original, no override,
...
//     but not the parameter semantics â€” CSceneAnimatableObject* this is
...
//     except inc the call counter â€” a regression here is impossible.
// ---------------------------------------------------------------------------
...
{
    // ---- Config ---------------------------------------------------------
    // Default OFF â€” Phase 1 is observation-only. Phase 2 will enable
    ...
    // ---- Original trampoline -------------------------------------------
    // Signature is conservative â€” RCX = scene object, the rest are
    ...
    // ---- Diagnostics ---------------------------------------------------
    // Reset the per-second counter â€” call from the watchdog/heartbeat
