...
// ---------------------------------------------------------------
// ESP â€” Modern overlay rendering with clean minimal aesthetics.
...
#include "../combat/aimbot.h"          // for Aimbot::diag_* live counters
...
namespace ESP
{
    ...
    inline Config cfg;
    inline bool   previewMode = false;   // true when menu open â†’ show preview panels

    // ---------------------------------------------------------------
    // Modern drawing primitives â€” clean lines, soft edges
    // ---------------------------------------------------------------
    namespace Draw
    {
                         ...
                         bool centered = false, float size = 0.f)
        {
            ...
            // Outer soft halo (8 directions, low alpha â†’ looks like a blurred
        ...
        {
            // Silk redesign â€” thin (2.4 px), full rounding, gradient sheen,
            ...
            // Trough â€” translucent slate
            dl->AddRectFilled(ImVec2(bx, y), ImVec2(bx + barW, y + h),
                              IM_COL32(14, 16, 22, 175), rnd);

            // Health colour â€” greenâ†’yellowâ†’red, slightly desaturated for "silk"
            float cr, cg;
            ...
            if (barH > 0.5f)
            {
                ...
                // Silk highlight â€” 1 px sheen down the left of the fill
                if (barH > 3.f)
                {
                    dl->AddLine(
                ...
                // Tiny bright cap dot at the top of the current fill â€” gives
            ...
            // HP number when not full â€” small dim chip above cap
            if (hp < 100)
            {
                              ...
                              bool defusing, float defuseRem, int style = 0)
        {
            ...
            // ----------------------------------------------------------
            //  STYLE 1 â€” VIVID  (compact accent capsule, glow halo)
            // ----------------------------------------------------------
            if (style == 1)
            {
            ...
            // ----------------------------------------------------------
            //  STYLE 2 â€” COMPACT  (small left-aligned chip)
            // ----------------------------------------------------------
            if (style == 2)
            {
            ...
            // ----------------------------------------------------------
            //  STYLE 0 â€” CLASSIC  (refined dark pill, centred)
            // ----------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // Skeleton bones (CS2 build 14152 â€” Animgraph 2 indices)
    ...
    //  27  VIEW_ANGLE (per UC â€” tracks aim direction; useful for resolver)
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // Visibility check â€” uses spotted state for ESP color switching
    // ---------------------------------------------------------------
    ...
    inline void Render()
    {
        ...
        // ---- Preview overlays â€” shown in lobby when menu is open ----
        ...
        if (cfg.bombTimer)
        {
            ...
            // Method A: dwPlantedC4 â†’ pointer â†’ entity pointer (raw pointer chain)
        ...
        auto drawDbg = [&]() {
            ...
            // Aimbot live state â€” diagnoses why the bot isn't moving the mouse.
            ...
            // SilentAim hook stage counters â€” pinpoints where the hook bails.
            ...
            } // end if(false) â€” debug overlay disabled for clean release build
        };
                ...
                if (weapSvc) {
                    ...
                    if (activeW) {
                        ...
                        if (cfg.weaponIcon && iconFont && !weaponName.empty())
                        {
                            ...
                            // Tighter scale (was 0.75) â€” icons were eating
        ...
        if (cfg.weapon && iconFont)
        {
            for (int i = 65; i < 2048; ++i)
            {
                ...
                // Tighter scale (was 0.9) â€” dropped icons no longer dominate
                ...
                // Type-tinted icon â€” desaturated a notch from the old neon
                ...
                // Subtle pill backdrop â€” gives the glyph a clean reading
        ...
        // Diagnostic readout â€” top-left, yellow.
        drawDbg();
    }
}
