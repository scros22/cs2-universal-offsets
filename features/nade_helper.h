...
// ---------------------------------------------------------------
// Nade Helper — shows guide overlay for common smoke/molotov
...
namespace NadeHelper
{
    ...
    {
        bool  enabled      = false;
        float triggerDist  = 120.f;  // units — how close to stand to see lineup
        float aimTolerance = 3.f;    // degrees — green when within this
    };
    inline Config cfg;
    ...
    // ---------------------------------------------------------------
    // Lineup database — popular competitive lineups
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // Map detection — read current map name from CGlobalVars
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    // Render — shows nearby lineups when player stands near spot
    // ---------------------------------------------------------------
    inline void Render()
    {
        ...
        for (int i = 0; i < kLineupCount; ++i)
        {
            ...
            // This lineup is active — render guide
            ...
            else
            {
                ...
                snprintf(aimTxt, sizeof(aimTxt), "Aim: %s %s (%.1f°)", vDir, hDir, aimDelta);
            }
            ...
            // Crosshair aim point — draw a small diamond at the target pitch/yaw
            // Convert target angles to a screen-ish position
            // (exact would require inverse viewmatrix — approximate with delta)
            if (!onTarget)
            {
