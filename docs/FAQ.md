...
The one in `manifest.json` and `GET /api/summary` — CS2 build 14184 at the time of writing. CS2 has two numbers: the game's build number (14184, what the console shows) and Steam's client build id (2000917). The site and the dump use the game's build number; release notes mention both.
...
Yes. `offsets.json` uses the same 32 names, patterns and semantics for the classic globals, and every value is cross-checked against a2x's dump of the same build before a release, so code written against a2x output works unchanged. The other ~170 globals only exist here (`offsets_all.json`, `offsets.hpp`).

**`patterns.json` fails to parse in my language.**
Since v2.1.4 it is strict JSON (`"rva": "0xB65A70"`). Older dumps wrote bare hex literals; if you are pinned to one of those, quote them (`s/: (0x[0-9A-F]+)/: "\1"/`) or use `GET /api/patterns`.
